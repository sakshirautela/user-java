...
public class MaximizetheNumberofTargetNodesAfterConnectingTreesI {
    public static void main(String[] args) {
        int[] res=maxTargetNodes(new int[][]{{}},new int[][]{{}},3);
        for (int i : res) {
            System.out.print(i+" ");
        }
    }
    public static int[] maxTargetNodes(int[][] edges1, int[][] edges2, int k) {
        ...
        int[] e1=buildGraph(edges1,k,n);
        int[] e2=buildGraph(edges2,k-1,m);
        int max2=0;
        for (int i : e2) {
    ...
    private static int[] buildGraph(int[][] edges, int k, int n) {
    ...
    private static int dfs(int src, int par,ArrayList<ArrayList<Integer>> graph, int k) {
        if(k<0){
            return 0;
        }
        int res=1;
