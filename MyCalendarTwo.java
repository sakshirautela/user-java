...
public class MyCalendarTwo {
    ...
    ArrayList<int[]> overlaping;
    public MyCalendarTwo() {
        booking=new ArrayList<int[]>();
        overlaping=new ArrayList<int[]>();
    }
    
    public boolean book(int start, int end) {
        for(int[] i:overlaping){
            if(isOverlaping(i,start,end)){
                return false;
            }
        }
        for(int[] i:booking){
            if(isOverlaping(i, start, end)){
                overlaping.add(getOverlaping(i,start,end));
            }
        }
    ...
    private int[] getOverlaping(int[] i, int start, int end) {
        return new int[]{Math.max(start,i[0]),Math.min(i[1],end)};

    }
    private boolean isOverlaping(int[] i, int start, int end) {
        return Math.max(start,i[0])<Math.min(i[1],end);
    }
}
