...
public class TheEarliestandLatestRoundsWherePlayersCompete {
    public static void main(String[] args) {
        int res[] = (earliestAndLatest(5, 1, 5));
        System.out.print(res[0] + " " + res[1]);
    }
    // The most possible playing round is ceil(log2(n))<=5.
    public int[] earliestAndLatestFast(int n, int firstPlayer, int secondPlayer) {
  ...
  public static int[] earliestAndLatest(int n, int firstPlayer, int secondPlayer) {
    ...
    return solve(firstPlayer, n - secondPlayer + 1, n, dp);
  }

  private static int[] solve(int l, int r, int k, int[][][][] dp) {
    if (l == r)
      return new int[] {1, 1};
    if (l > r)
      return solve(r, l, k, dp);
    ...
    for (int i = 1; i <= l; ++i)
      for (int j = l - i + 1; j <= r - i; ++j) {
        if (i + j > (k + 1) / 2 || i + j < l + r - k / 2)
          continue;
        int[] res = solve(i, j, (k + 1) / 2, dp);
        min = Math.min(min, res[0] + 1);
        max = Math.max(max, res[1] + 1);
      }

    return dp[l][r][k] = new int[] {min, max};
  }
}
